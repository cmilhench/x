...
func TestMessage(t *testing.T) {
	var tests = []struct {
		...
		{"3", ":user!~mail@example.net JOIN #express\r\n", Message{"user!~mail@example.net", "JOIN", "#express", "", ""}},
		{"4", ":user!~mail@example.com PRIVMSG user :Hello :)\r\n", Message{"user!~mail@example.com", "PRIVMSG", "user", "Hello :)", ""}},
		{"6", ":user!~mail@example.com PRIVMSG #chan :Hello :)\r\n", Message{"user!~mail@example.com", "PRIVMSG", "#chan", "Hello :)", ""}},
